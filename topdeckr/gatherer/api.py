...
class CardDatabase(object):
	...
	def online_lookup_by_name(self, name):
		...
		if cached_card is None:
			...
			f.write(xml.read())
			f.close()
			new_cached_card = self._parse_gatherer_xml(open(cached_filename, 'r'))
			new_cached_card = Card(**new_cached_card)
			new_cached_card.save()
			return new_cached_card.toDict()
		else:
			return cached_card.toDict()
	...
	def _parse_gatherer_xml(self, xml):
		...
		for table in tree.findall('tbody'):
			print lxml.etree.tostring(table.text())
